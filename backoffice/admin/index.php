    ...
    try {
        if ($action === 'save_product') {
            ...
            if ($id > 0) {
                ...
                $password = $_POST['password'] ?: 'nocibe123';
...
$lowStock = $pdo->query('SELECT nom_produit, quantite_stock FROM produit WHERE quantite_stock <= 5 ORDER BY quantite_stock ASC LIMIT 8')->fetchAll();
...
$dashboardLead = 'Statistiques, commandes, produits, paiements, livraisons et utilisateurs.';
...
<section class="dashboard-summary-grid">
    <article class="metric-card"><strong><?= $stats['orders'] ?></strong><span>Commandes</span></article>
    <article class="metric-card"><strong><?= number_format($stats['revenue'], 0, ',', ' ') ?> FCFA</strong><span>Chiffre d'affaires</span></article>
    <article class="metric-card"><strong><?= $stats['deliveries'] ?></strong><span>Livraisons en cours</span></article>
    <article class="metric-card"><strong><?= $stats['clients'] ?></strong><span>Clients</span></article>
</section>
            ...
            <div class="toolbar">
                ...
                <form class="inline-form" method="GET">
                    <select class="form-select" name="statut">
                        ...
                        <?php foreach (['En attente', 'Confirmee', 'En livraison', 'Livree', 'Annulee'] as $status): ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <form class="inline-form" method="POST">
                                    ...
                                    <button class="icon-btn" title="Changer le statut"><i class="fa-solid fa-check"></i></button>
                                </form>
                            </td>
                        </tr>
                    <?php endforeach; ?>
...
<section class="workspace-grid dashboard-section" id="produits">
    <article class="panel-card">
                    ...
                    <tr>
                        ...
                        <td>
                            <form class="inline-form" method="POST">
                                ...
                                <button class="icon-btn icon-btn-danger" title="Supprimer"><i class="fa-solid fa-trash"></i></button>
                            </form>
                        </td>
                    </tr>
                <?php endforeach; ?>
    ...
    <aside class="panel-stack">
        <article class="panel-card product-form-card">
            ...
            <form class="compact-form" method="POST">
                ...
                <input class="form-control" name="id_produit" placeholder="ID produit a modifier (laisser vide pour ajouter)">
                ...
                <?php foreach ($categories as $category): ?>
                    <li>
                        ...
                        <form method="POST">
                            ...
                            <button class="icon-btn icon-btn-danger"><i class="fa-solid fa-trash"></i></button>
                        </form>
                    </li>
                <?php endforeach; ?>
            </ul>
...
<section class="workspace-grid dashboard-section" id="paiements">
        ...
        <div class="table-shell">
            <table class="table-modern">
                ...
                <?php foreach ($payments as $payment): ?>
                    <tr><td>#<?= (int) $payment['id_paiement'] ?></td><td>#<?= (int) $payment['id_commande'] ?></td><td><?= htmlspecialchars($payment['mode_paiement']) ?></td><td><?= htmlspecialchars($payment['date_paiement']) ?></td><td><?= htmlspecialchars($payment['statut_paiement']) ?></td></tr>
                <?php endforeach; ?>
                </tbody>
            </table>
        </div>
            ...
            <table class="table-modern">
                ...
                <?php foreach ($deliveries as $delivery): ?>
                    <tr>
                        ...
                        <td><?= htmlspecialchars($delivery['statut_livraison']) ?></td>
                        <td>
                            <form class="inline-form" method="POST">
                                ...
                                <button class="icon-btn"><i class="fa-solid fa-check"></i></button>
                            </form>
                        </td>
                    </tr>
                <?php endforeach; ?>
...
<section class="workspace-grid dashboard-section" id="utilisateurs">
    <article class="panel-card">
            ...
            <table class="table-modern">
                ...
                <?php foreach ($users as $user): ?>
                    <tr>
                        ...
                        <td><?= htmlspecialchars(userRole($pdo, (int) $user['id_user'])) ?></td>
                        <td><?= ((int) ($user['is_active'] ?? 1)) === 1 ? 'Actif' : 'Desactive' ?></td>
                        <td class="row-actions">
                            <form method="POST"><input type="hidden" name="action" value="toggle_user"><input type="hidden" name="id_user" value="<?= (int) $user['id_user'] ?>"><button class="icon-btn" title="Activer/desactiver"><i class="fa-solid fa-power-off"></i></button></form>
                            <form method="POST"><input type="hidden" name="action" value="delete_user"><input type="hidden" name="id_user" value="<?= (int) $user['id_user'] ?>"><button class="icon-btn icon-btn-danger" title="Supprimer"><i class="fa-solid fa-trash"></i></button></form>
                        </td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
    ...
    <aside class="panel-card">
        ...
        <form class="compact-form" method="POST">
            ...
            <input class="form-control" type="password" name="password" placeholder="Mot de passe (nouveau compte)">
