...
} elseif (!empty($_SESSION['user_id']) || !empty($_SESSION['user'])) {
    $sessionRole = $_SESSION['role'] ?? ($_SESSION['user']['role'] ?? 'client');
    header('Location: ' . roleRedirectPath((string) $sessionRole));
    exit;
}
?>
<!DOCTYPE html>
